...
export default class SlidecontrolEngine {
  ...
  start(Connection, presentationID) {
    ...
    chrome.runtime.sendMessage(
      "Your code for this presentation is " + presentationID,
    );
    ...
    const qrButtonContainer = document.querySelector(
      "#slidecontrol-qr-button-block",
    );
    const googleSlideButton = document.querySelector(
    ...
    idContainer.addEventListener("click", () =>
      this.watermark.toggle(presentationID),
    );

    // add button for QR-Code
    qrButtonContainer.style.display = "inline-block";
    qrButtonContainer.addEventListener("click", () =>
      this.qrCodeWindow.toggle(),
    );
  ...
  get presentationData() {
    ...
    viewerDataScript.textContent =
      "document.querySelector('body').setAttribute('viewerData', JSON.stringify(viewerData))";
    ...
    const activeSlide = parseInt(
      googleSlideButton.getAttribute("aria-posinset"),
    ); // current Slide
    const totalSlides = parseInt(
      googleSlideButton.getAttribute("aria-setsize"),
    ); // total Slides
  ...
  handleNotification(notification) {
    ...
    const typeMap = {
      ...
      "laserpointer-move": () =>
        this.laserpointer.move(notification.x, notification.y),
