...
module.exports = class SlidecontrolEngine {
  ...
  closeConnection(connection) {
    if (connection.__type__ === "presentation")
      this.removePresentation(connection);
    else if (connection.__type__ === "controller")
      this.removeController(connection);
  }
  ...
  removePresentation(connection) {
    ...
    this.presentations[connection.__presentationID__].controller.forEach(
      (controller) => {
    ...
    Logger.debug(
      "Removed Presentationw with ID:",
      connection.__presentationID__,
    );
  }
  ...
  removeController(connection) {
    ...
    const index =
      this.presentations[connection.__presentationID__].controller.indexOf(
        connection,
      );
    if (index >= 0) {
      this.presentations[connection.__presentationID__].controller.splice(
        index,
        1,
      );
    }
  ...
  addNewPresentation(data, connection) {
    ...
    const presentationID = generatePresentationID(
      this.presentations,
      parseInt(process.env.PRESENTATION_ID_LENGTH) || 5,
    );
  ...
  checkPresentationID(data, connection) {
    ...
    sendData(connection, {
      command: this.presentations[data.presentationID]
        ? "presentation-id-ok"
        : "presentation-id-unknown",
      data: {
  ...
  addNewController(data, connection) {
    ...
    if (!data.presentationID) return;
    if (connection.__presentationID__) return;
    if (!this.presentations[data.presentationID])
