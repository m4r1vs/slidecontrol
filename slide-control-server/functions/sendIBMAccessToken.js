...
module.exports = sendIBMAccessToken = (req, res) => {
  Logger.debug("Will get IBM access_token, with key: ", `"${process.env.IBM_API_KEY}"`);

  // get token from IBM and send as response
  fetch(
    "https://iam.bluemix.net/identity/token?grant_type=urn:ibm:params:oauth:grant-type:apikey&apikey=" +
      IBM_API_KEY,
    {
      method: "POST",
      mode: "cors",
    ...
    .catch((error) =>
      Logger.error("Error getting access_token from IBM server: ", error),
    );
};
